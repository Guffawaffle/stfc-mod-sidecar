...
describe("viewer landing page markup", () => {
    ...
    test("keeps Setup as a thin static landing page over existing setup flows", () => {
        expect(setupHtml).toContain('data-current-page="setup"');
        expect(setupHtml).toContain("<h1>Setup</h1>");
        expect(setupHtml).toContain("Companion Settings");
        expect(setupHtml).toContain('href="/settings/"');
        expect(setupHtml).toContain('href="/about/"');
        expect(setupHtml).toContain('href="/settings/#notifications"');
        expect(setupHtml).toContain('/shared/shell.js');
        expect(setupHtml).not.toContain('/setup/app.js');
    });
