...
When SQL storage is available, viewer APIs treat it as the authoritative local data layer. Empty SQL scopes return empty store snapshots instead of falling back to older JSONL files, so stale fallback data cannot masquerade as live data.

The sidecar should not reach into the game process. The mod should not depend on the sidecar being present.

The architecture contract for that boundary is [docs/22-producer-consumer-security-contract.md](22-producer-consumer-security-contract.md). In particular, sidecar availability must never become a gameplay requirement, and consumer backpressure must be handled by dropping, sampling, degrading, or disabling optional export rather than blocking the mod.
...
- the desktop companion stores SQLite under Electron `userData` unless `STFC_SIDECAR_STORE_CONNECTION` is explicitly provided
...
- local token: external ingest clients should set `STFC_SIDECAR_SYNC_TOKEN`;
  ...
  is unset. This is a local ingest key only, not a provider credential.
- remote token: remote targets use a target-scoped key generated by the remote
  provider or explicit user config. Do not reuse the local sidecar sync key for
  remote egress.
